...
from tools.lib.model_utils import load_musicgen
...
def analyze_spectrum(audio, sample_rate):
    ...
    bands = [
        (0, 60, "Sub-bass", "rumble, hum"),
        (60, 250, "Bass", "warmth, boom, thump"),
        (250, 2000, "Mid", "body, melody, instruments"),
        (2000, 6000, "Presence", "clarity, detail, voice"),
        (6000, sample_rate // 2, "Brilliance", "air, sparkle, shimmer"),
    ]
    band_data = []
    ...
    mid_pres = sum(pct for _, _, name, _, pct in band_data if name in ("Mid", "Presence"))
    return band_data, mid_pres
...
def print_analysis(audio, sample_rate, highpass_hz, file_size, gen_time, device):
    ...
    console.print()
    console.print("  [bold dim]Dynamics (RMS per second)[/bold dim]")
    console.print("  [dim]────────────────[/dim]")

    if chunk_rms:
        max_rms = max(chunk_rms)
        for i, crms in enumerate(chunk_rms):
            bar_len = int(30 * crms / max_rms) if max_rms > 0 else 0
            bar = "▓" * bar_len + "░" * (30 - bar_len)
            console.print(f"  {i:2d}s  {bar}  {crms:.4f}")
...
def main():
    ...
    parser.add_argument("--model-size", choices=["small", "medium", "large"], default="medium",
                        help="Model size: small (300M), medium (1.5B), large (3.3B) (default: medium)")
    parser.add_argument("--temperature", type=float, default=1.0,
                        help="Sampling temperature — higher = more creative (default: 1.0)")
    ...
    parser.add_argument("--output", default=None, help="Output file path (default: ./tmp/gen_music/output.wav)")
    args = parser.parse_args()

    output = args.output or os.path.join(".", "tmp", "gen_music", "output.wav")
    os.makedirs(os.path.dirname(output), exist_ok=True)

    console.print()
    console.print("[bold cyan]🎵 gen-music[/bold cyan]")
    console.print()
    ...
    params.add_row("Model", f"[cyan]musicgen-{args.model_size}[/cyan]")
    ...
    t0 = time.time()
    model, processor = run_with_hf_fallback(load_musicgen, args.model_size)
    ...
    if args.count > 1:
        ...
        rank_table.add_column("Mid+Pres", justify="right")
